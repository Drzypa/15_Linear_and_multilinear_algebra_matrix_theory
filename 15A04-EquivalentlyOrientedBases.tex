...
\usepackage{pmmeta}
\pmcanonicalname{EquivalentlyOrientedBases}
\pmcreated{2013-03-22 14:01:01}
\pmmodified{2013-03-22 14:01:01}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{equivalently oriented bases}
\pmrecord{5}{34928}
\pmprivacy{1}
...
\item $(v_1,\ldots ,v_n)$ ordered basis for $V$
\item $(w_1,\ldots ,w_n)$ ordered basis for $V$
\item $A:V\rightarrow V$
\item $\forall i\in \{1,\ldots ,n\}: Av_i=w_i$
\item $\det(A)>0$ 
\end{123listcolonstyle}

\section*{fact: there is a unique linear isomorphism taking a given basis to another given basis}

\begin{123listcolonstyle} 
\item $V$ finite-dimensional vector space
\item $(v_1,\ldots ,v_n)$ ordered basis for $V$
\item $(w_1,\ldots ,w_n)$ ordered basis for $V$
\item $\exists ! A:V\rightarrow V$ linear isomorphism $
